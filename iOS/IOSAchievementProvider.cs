...
{
    /// <summary>
    /// Achievement provider for iOS backend.
    /// Uses process-local shared state with optional Game Center projection.
    /// </summary>
    public sealed class IOSAchievementProvider : IAchievementProvider
    {
        ...
        public async Task<AchievementCollection> GetAchievementsAsync(SignedInGamer gamer, CancellationToken cancellationToken = default)
        {
            ...
            IReadOnlyDictionary<string, AppleGameCenterAchievementProgress> liveProgress = null;
        ...
        public async Task SetProgressAsync(SignedInGamer gamer, string achievementKey, float percentComplete, CancellationToken cancellationToken = default)
        {
            ...
            if (IOSRuntime.TryGetAppleGameCenterClient(out var client))
            {
                try
                {
                    await client.ReportProgressAsync(achievementKey, percentComplete, cancellationToken).ConfigureAwait(false);
                }
                catch
                {
